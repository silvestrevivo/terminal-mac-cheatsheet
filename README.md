...
The terminal has two first things to learn: _The prompt_ `$` is where you type the commands to execute, and the _root_ `~` is just the **root user folder**. There are _three_ types of commands:

- Single: `$ date` => _$ command_

- With extra: `$ echo "message"` => _$ command + extra_

- With action and something extra: `$ cal 01 2018` => _$ command + (-option) + (something extra)_
...
|          Ctrl + A          | Goes to the beginning of the line where you are currently working on |
|          Ctrl + E          | Goes to the end of the line where you are currently working on       |
|     Ctrl + Q, Ctrl + U     | Clears the current line                                              |
| Ctrl + W, Cmnd + Shift + ← | Cuts one word backward using white space as delimiter                |
|          Ctrl + K          | Clears the line after cursor                                         |
|          Ctrl + L          | Cleans the terminal no deleting content                              |
|          Cmnd + K          | Cleans de terminal deleting content                                  |
|          Ctrl + C          | Kills whatever you are runing                                        |
|          Alt + →           | Moves cursor one word forward                                        |
|          Alt + ←           | Moves cursor one word backward                                       |
...
|  cd, cd ~   | Moves to user directory           |
|    cd /     | Moves to root of drive directory  |
| cd [folder] | Gets into directory               |
|    cd -     | Goes to previous directory        |
|     pwd     | Gets current path directory       |
...
|    ls -A    | Listing files sorted in alphabetical order  |
...
| mkdir [dir] && cd [dir] | Creates and enters in new directory                         |
|  mkdir -p [dir]/[dir]   | Creates nested directories                                  |
|       rmdir [dir]       | Removes directory ( only operates on empty directories )    |
|   [command] > [file]    | Pushes output to file, keep in mind it will get overwritten |
|   [command] >> [file]   | Appends output to existing file                             |
|   [command] < [file]    | Tells command to read content from a file                   |
...
|       less [file]       | Output file content delivered in another space in the terminal |
...
|         pbpaste         | Pastes clipboard contents                                      |
|    pbpaste > [file]     | Pastes clipboard contents into file                            |
...
|             Key/Command              | Description                                                                                   |
| :----------------------------------: | --------------------------------------------------------------------------------------------- |
|                find .                | Searchs everything in the current folder                                                      |
|  find [dir] -name [search_pattern]   | Searchs for files inside a folder                                                             |
|          find [dir] -type f          | Finds just the files inside a folder                                                          |
|          find [dir] -type d          | Finds just the folders inside a folder                                                        |
|       mdfind [search_pattern]        | Spotlights search for files (names, content, other metadata)                                  |
| mdfind -onlyin [dir] -name [pattern] | Spotlights search for files named like pattern in the given directory                         |
...
|    grep -v [search_pattern][file]    | Searchs for all lines that do NOT contain the pattern                                         |
...
|   ctrl-r    | Interactively searchs through previously typed commands               |
|  ![value]   | Executes the last command typed that starts with `value`              |
| ![value]:p  | Prints to the console the last command typed that starts with `value` |
|     !!      | Executes the last command typed                                       |
|    !!:p     | Prints to the console the last command typed                          |
...
|          man [command]          | Shows the help manual for [command]               |
